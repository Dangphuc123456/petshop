        ...
        @foreach($orders as $order)
            <div class="order-card" id="order-{{ $order->order_id }}">
                <div class="orders-status">
                    Trạng thái | <span class="status">{{ $order->status }}</span>
                </div>
                <hr style="margin: 5px 0;">
                @foreach($order->orderItems as $item)
                    <div class="order-item" id="item-{{ $item->order_item_id }}">
                        <img src="{{ asset('anh/' . $item->image_url) }}" 
                ...
                <div class="order-footer">
                    <p class="total-price">
                        Thành tiền: <span>{{ number_format($order->total_amount, 0, ',', '.') }} VNĐ</span>
                    </p>
